...
class Courses extends React.Component {
	constructor() {
		...
		this.state = {
			...
			filters: [
				// top
				{ id: 'openstore', label: '開店', type: 'course' },
				{ id: 'foreigncook', label: '西餐', type: 'course' },
				{ id: 'conditioner', label: '冷氣', type: 'course' },
				{ id: 'media', label: '多媒體', type: 'course' },
				{ id: 'welding', label: '焊接', type: 'course' },
				{ id: 'web', label: '網站', type: 'course' },
				{ id: 'hair-design', label: '美髮', type: 'course' },
				{ id: 'cloth', label: '縫紉', type: 'course' },
				{ id: 'marketing', label: '行銷', type: 'course' },
				{ id: 'engineer', label: '工程師', type: 'course' },
				{ id: 'social', label: '社群', type: 'course' },
			...
			inputValue: '',
		}; // middle-right這個是location(職訓局的位置) // fullday
		...
		return (
			<div onClick={this.showSelect}>
					...
					<div className="col-lg-12 col-md-12 col-12 top__keyword__box py-3">
						{this.state.filters
							...
							.map(job => {
								return (
									<button
										className={'mt-3 mx-2 px-2 tagbtn ' + (job.isActive ? 'clicked' : 'unclick')}
										name={job.id}
								...
								{this.state.filters
									...
									.map(job => {
										return (
											<button
												className={
													'mt-3 mx-2 px-2  tagbtn ' + (job.isActive ? 'clicked' : 'unclick')
												}
												name={job.id}
												onClick={this.handleClick}
											>
						...
						.map(job => {
							return (
								<button className="mx-2 px-2  tagbtn clicked" name={job.id} onClick={this.handleClick}>
									{job.label}
									<i className="fas fa-times delete" onClick={this.handleClick} />
								</button>
							);
						})}
				</div>
				...
				<div className="p-2 mt-3 course__result__box col-lg-12 col-md-12 col-12">
					{!this.state.result &&
						this.state.defaultResult.map(course => (
							<div className="mb-3 course__box d-flex flex-wrap">
								<div className="course__pic col-lg-4 col-md-4 col-12">
									...
									<img
										className="img-fluid"
										src="http://t.wsgxsp.com/uploads/150431955713113.png"
										alt=""
									/>
								</div>
								<div className="course__details col-lg-8 col-md-8 col-12 py-2">
									<h4 className="course__name">{course.train_class}</h4>
									<p className="course__place">
										<i className="fas fa-map-marked-alt mr-2" />
										{course.train_place}
									</p>
									<p className="sub__title course__time mb-0">
										<i className="fas fa-calendar-alt mr-2" />
										{course.train_time}
									</p>
									<p className="sub__title course__period mb-0">
										<i className="fas fa-school mr-2" />
										{course.train_period}
									</p>
									<p className="sub__title course__job__name mb-0">
